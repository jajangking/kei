...
// =======================
// ARDUINOJSON COMPATIBILITY (Auto-detect v6 or v7)
// =======================
...
void handleWiFi() {
  ...
  if (wifiConnecting && millis() - wifiConnectStart > WIFI_TIMEOUT) {
    WiFi.disconnect();
    wifiConnecting = false;
  }
  if (!wifiConnecting && millis() - lastWifiAttempt > 5000) {
...
void mqttPublishTelemetry() {
  if (!mqttEnabled) return;
  if (!mqttClient.connected()) return;
  mqttClient.publish(getTeleTopic().c_str(), buildTelemetryJson().c_str(), true);
}
...
void handleMessage(String msg) {
  ...
  if (doc["leftTrim"].is<int>()) {
    leftTrim = constrain(doc["leftTrim"], -100, 100);
  }
  if (doc["rightTrim"].is<int>()) {
    rightTrim = constrain(doc["rightTrim"], -100, 100);
  }
  ...
  if (doc["mqttBroker"].is<String>()) {
    saveMqttConfig(
      doc["mqttBroker"], doc["mqttPort"] | 8883,
      doc["mqttUser"] | "", doc["mqttPass"] | "",
      doc["mqttPrefix"] | "kei/robot", doc["mqttEnabled"] | true
    );
  ...
  if (doc["leftMotor"].is<int>() || doc["rightMotor"].is<int>()) {
    ...
    targetLeftSpeed = constrain((doc["leftMotor"] | 0) + leftTrim, -cap, cap);
    targetRightSpeed = constrain((doc["rightMotor"] | 0) + rightTrim, -cap, cap);
...
void rampMotors() {
  if (currentLeftSpeed != targetLeftSpeed) {
    int step = (targetLeftSpeed > currentLeftSpeed) ? rampRate : -rampRate;
    currentLeftSpeed += step;
    if (abs(currentLeftSpeed - targetLeftSpeed) < rampRate) {
      currentLeftSpeed = targetLeftSpeed;
    }
    currentLeftSpeed = constrain(currentLeftSpeed, -255, 255);
    writeMotorA(currentLeftSpeed);
  }

  if (currentRightSpeed != targetRightSpeed) {
    int step = (targetRightSpeed > currentRightSpeed) ? rampRate : -rampRate;
    currentRightSpeed += step;
    if (abs(currentRightSpeed - targetRightSpeed) < rampRate) {
      currentRightSpeed = targetRightSpeed;
    }
    currentRightSpeed = constrain(currentRightSpeed, -255, 255);
