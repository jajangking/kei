...
void loop() {
  ...
  if (getBehavior() == "stop" && safetyActive) { ledcWrite(CH_LEFT, 0); ledcWrite(CH_RIGHT, 0); currentLeftSpeed = 0; currentRightSpeed = 0; }
...
void handleMessage(String msg) {
  ...
  if (doc["exploreSpeed"].is<int>()) { setExploreSpeed(doc["exploreSpeed"].as<int>()); }
...
String buildTelemetryJson() {
  ...
  j += ",\"exploreSpeed\":" + String(getExploreSpeed());
